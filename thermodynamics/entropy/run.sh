...
$PARENTDIR/scripts/init.sh $PARAMS
...
$PARENTDIR/exec/BAT_builder.x ../${TOP} ../${TRJ} ${NAME}.bat "${BACKBONE_ATOMS}" double_precision >> log.txt 2>&1
...
mpirun -bynode -cpus-per-proc $OMP_NUM_THREADS -np $MPI_NUM_PROCS --mca btl tcp,self,sm --report-bindings $PARENTDIR/exec/PARENT.x ${NAME}.bat ${NAME}.par ${BBINS1D} ${ABINS1D} ${DBINS1D} ${BBINS2D} ${ABINS2D} ${DBINS2D} >> log.txt 2>&1
...
$PARENTDIR/exec/get_PAR_info.x ${NAME}.par > ${NAME}_MIE_topology.txt 2>> log.txt
...
$PARENTDIR/exec/get_PAR_MIE.x ${NAME}.par > ${NAME}_MIE.txt 2>> log.txt

# Run the calculation of the MIST approximation specifying the input .par file and the output .txt file
# Depending on the architecture of your cluster/workstation as well as your Open MPI version, you might want to change the mpirun parameters. Especially if you are using InfiniBand you might want to change "tcp" to "openib"
mpirun -bynode -cpus-per-proc $OMP_NUM_THREADS -np $MPI_NUM_PROCS --mca btl tcp,self,sm --report-bindings $PARENTDIR/exec/get_PAR_MIST.x ${NAME}.par ${NAME}_MIST.txt >> log.txt 2>&1
